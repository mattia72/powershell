...
function Get-Icon {
  ...
  param (
    ...
    [string]$Path,
    [string]$Index = 0,
    [IconSizeType] $Size = [IconSizeType]::Large
  )
    
  begin {

    Add-Type -Path $PSScriptRoot\..\IconExtractor\bin\Debug\net5.0\IconExtractor.dll -ReferencedAssemblies System.Drawing,System.IO,System.Runtime.InteropServices -ErrorAction SilentlyContinue
    # Ensure file exists
    $Path = Resolve-Path $Path
  ...
  process {
      [IconExtractor.IconExtractor]::Extract($Path, $Index, 
        $Size -eq [IconSizeType]::Large ? [IconExtractor.IconSizeType]::Large : [IconExtractor.IconSizeType]::Small)  
  }
...
Export-ModuleMember -Function Get-Icon, Export-AllIcons