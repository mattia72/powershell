...
function Read-AndWriteElementValueFilteredXml {
  param (
    [ValidateScript( {(Test-Path -Path $_) })]
    $Src,
    $Dest,
      ...
      switch ($reader.NodeType) {
        'Element' { 
          if ($parent_started) {
            ...
            $parent_started = ( $reader.Value -eq $Parent) 
...
function Read-AndWriteElementNameFilteredXml {
  param (
    [ValidateScript( {(Test-Path -Path $_) })]
    $Src,
    $Dest,
...
Read-AndWriteElementNameFilteredXml -Src $XmlSrcPath -Dest $XmlDestPath -Filter $ElementFilter -Parent $ParentElement
